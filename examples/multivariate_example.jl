...
fig
  ...
  # ============================================================================
  # Part 9: Test AGP_SEP (separable local GP) on this anisotropic function
  # ============================================================================

  println("\n" * "="^60)
  println("AGP_SEP (Local Approximate GP - Separable)")
  println("="^60)

  # Use darg to get reasonable isotropic lengthscale bounds
  d_agp = darg(X)
  g_agp = garg(y)

  # Run AGP with MLE for local hyperparameters
  pred_agp = agp_sep(X, y, XX;
      start=6, endpt=50,
      d=(start=d_agp.start, min=d_agp.min, max=d_agp.max, mle=true),
      g=(start=g_agp.start, min=g_agp.min, max=g_agp.max, mle=true),
      method=:alc, verb=0
  )

  mean_agp_grid = reshape(pred_agp.mean, n_x1, n_x2)
  std_agp_grid = reshape(sqrt.(pred_agp.var), n_x1, n_x2)

  println("AGP predictions complete:")
  println("  Mean range: [$(round(minimum(pred_agp.mean), sigdigits=3)), $(round(maximum(pred_agp.mean), sigdigits=3))]")
  println("  Local d range: [$(round(minimum(pred_agp.mle.d), sigdigits=3)), $(round(maximum(pred_agp.mle.d), sigdigits=3))]")
  println("  Local g range: [$(round(minimum(pred_agp.mle.g), sigdigits=3)), $(round(maximum(pred_agp.mle.g), sigdigits=3))]")

  # Compare to true function
  agp_rmse = sqrt(mean((pred_agp.mean .- y_true).^2))
  iso_rmse = sqrt(mean((pred_iso.mean .- y_true).^2))
  sep_rmse = sqrt(mean((pred_sep.mean .- y_true).^2))

  println("\nRMSE comparison (vs true function):")
  println("  Isotropic GP:  $(round(iso_rmse, sigdigits=4))")
  println("  AGP (local):   $(round(agp_rmse, sigdigits=4))")
  println("  Separable GP:  $(round(sep_rmse, sigdigits=4))")

  # Add AGP to visualization
  fig_agp = Figure(size=(1100, 500))

  ax1 = Axis(fig_agp[1, 1], xlabel="x1", ylabel="x2", title="AGP: Mean")
  hm1 = heatmap!(ax1, collect(x1_grid), collect(x2_grid), mean_agp_grid',
                 colormap=:viridis, colorrange=clims)
  scatter!(ax1, X[:, 1], X[:, 2], color=:white, markersize=4,
           strokecolor=:black, strokewidth=0.5)
  Colorbar(fig_agp[1, 2], hm1, label="Mean")

  ax2 = Axis(fig_agp[1, 3], xlabel="x1", ylabel="x2", title="AGP: Uncertainty (Std)")
  hm2 = heatmap!(ax2, collect(x1_grid), collect(x2_grid), std_agp_grid',
                 colormap=:plasma)
  scatter!(ax2, X[:, 1], X[:, 2], color=:white, markersize=4,
           strokecolor=:black, strokewidth=0.5)
  Colorbar(fig_agp[1, 4], hm2, label="Std")

  Label(fig_agp[0, :], "AGP_SEP (Separable Local GP) on Anisotropic Function", fontsize=14)

  save(joinpath(@__DIR__, "multivariate_agp.png"), fig_agp, px_per_unit=2)
  println("\nAGP figure saved to: ", joinpath(@__DIR__, "multivariate_agp.png"))
